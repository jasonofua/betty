...
def parse_bookings():
    ...
    for path in paths:
        ...
        for i, m in enumerate(heads):
            ...
            seen[code] = dict(code=code, when=m.group(1), label=lab, product=product_of(lab), sport=(None if product_of(lab) in ('Bet of the day', 'Rollover', 'By market') else sport_of(lab)),
...
# 25 Sep, user's call: NO leg cap. "The bet of the day can be as much as it wants
# to be, the issue is the individual legs." The slip is as big as the qualifying
# families make it; the work belongs in the per-fixture read, not in trimming.
BEST_MAX_LEGS = A.MAX_CODE
...
def rollover_of_day(dry=False):
    ...
    Measured on our own boards 14-25 Sep: a 1.5x slip could be built on 8 of the
    12 days and won 7. The day is skipped rather than reached for when the safe
    families cannot get there - a 1.5x built out of 60% legs is not the same bet."""""
    today = dt.datetime.now(tz=WAT).date(); now = time.time()
    fs = family_stats()
...
CHANGELOG = [
    dict(date='26 Sep', txt="First-half Unders, scanned off the board instead of through the evaluator. On the 26 Sep board the Under 2.5 scan went 30 of 32 (94%) - 13 of 14 where both sides' records said 100%, 10 of 11 where they said 90-99%. The Under 1.5 version of the same scan went 13 of 20 (65%) and its ordering was inverted: the legs it rated 90%+ went 1 of 2, the legs it rated under 80% went 4 of 4. One goal decides a first half at 1.5, so counting past first halves that stayed under it measures the league's base rate, not the fixture. Under 2.5 now runs daily at 10:02 over every game in the window whose two sides both clear 90%; Under 1.5 is not booked until there is a real expected-goals read behind it."),
    dict(date='26 Sep', txt="Points-sports handicaps now need the expected margin to clear the line, not just a tally of past games. The agreement count on its own does not predict - on our own settled legs it ran 11 of 14 -> 65%, 12 of 14 -> 59%, 14 of 14 -> 55%. Counting how many past games happened to clear a line throws the margin away: Karlovy Vary's home results were 0:2 3:0 2:3 4:0 1:4 0:3 5:2, which the tally reads as 4 of 7 inside 1.5 when what it says is that their results scatter by five goals, and they won 5:1. The line must now also be cleared by the two sides' expected margin by more than 1.25 standard deviations of that scatter. Backtested leak-free on 198 hockey fixtures with the venue windows cut at each match date: the old rule hit 351/442 = 79%, the new one 199/233 = 85%, and it holds on both halves of the sample (86% older, 85% newer, against 82% and 78%)."),
    dict(date='26 Sep', txt="One market per slip (user's call): a 09:58 run that files every supported option on the board by the option itself and books a code for each - all the Over 1.5 games on one slip, all the 1st Half Over 0.5 games on another, all the 2nd Half Under 2.5 games on a third. Football only, one leg per match, any option two or more matches support becomes a slip, up to 50 legs. A game reaches a slip only because its own two teams' records made that option one of its three best - the market is never chosen first and then filled."),
    dict(date='25 Sep', txt="Checked and rejected after the same losses, so they are not in: a winners venue margin carried by one big win (49/59 = 83%, against 21/26 = 81% when it holds without that win - Lunds' +1.00 was a 7:1 and it still tests as nothing); points handicaps where one side's own column is weak (4/6); a price cap on points legs (1.35-1.60 is 22/35, 1.60-1.90 is 42/68, 1.90+ is 14/21 - no band to cut). Points totals looked bad at 43/79 before 20 Sep but are 7/7 since the season gate went in, so that rule is already doing its job."),
...
SCHEDULE = [
    ...
    ('10:02', 'h1-under-2.5', '/api/h1unders', dict(until=23, days=0)),
