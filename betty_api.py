...
RULES = [
    ...
    dict(product='Draws', tag='the band, the table, the Under',
         plain="One slip a day of draws from the market's draw band: SportyBet's own price puts the draw at 27% or more after the overround, the two sides are close in the league table THIS season (points a game within 0.5, both with 8+ league games - a league two rounds old has no table and is skipped), the home side is not the weaker one, and Under 2.5 goals is 1.70 or shorter. Main-league games where SportyBet's draw price is 5% or more above the market average are added on the price alone. The old gate - venue form, expected goals, combined draws - is gone: on 78,000 priced matches it added nothing over the price.",
         thresholds=[dict(k='SportyBet-implied draw', v='27% or more'), dict(k='Table', v='points a game within 0.5, 8+ league games each'), dict(k='Home side', v='not 10+ points weaker on the 1X2'), dict(k='Under 2.5', v='1.70 or shorter'), dict(k='Shape', v='one slip, up to 12 legs')],
