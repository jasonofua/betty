...
RULES = [
    ...
    dict(product='Draws', tag='the price, not the pick',
         plain="Backing the draw where SportyBet's price sits 5% or more above the market average and the market has the draw at 30-36%, in the leagues football-data prices (22 divisions). Singles. The old gate - venue form, expected goals, combined draws - is gone: on 78,000 priced matches it added nothing over the price (quiet-gate games -3.7% at closing, the rest -4.1%).",
         thresholds=[dict(k='Market-implied draw', v='30-36%'), dict(k='SportyBet price v market average', v='1.05x or more'), dict(k='Shape', v='singles, main leagues only')],
         measured='football-data 2015-26: +14.6% and +10.6% on the two halves of the sample (n 321 / 379); with Pinnacle as the reference in the 32-36% band, +5.7% / +11.4% (n 1,351 / 1,320). The market itself is calibrated on draws in every band.'),
